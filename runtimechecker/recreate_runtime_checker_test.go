	...
	Describe("Check", func() {
		...
		It("fails if the BOSH version is below the minimum requirement", func() {
			for _, version := range []string{"265.0.0", "266.14.9", "267.8.9", "268.2.1", "268.3.9"} {
				checker := runtimechecker.RecreateRuntimeChecker{BoshInfo: boshdirector.Info{Version: version}}
				Expect(checker.Check()).To(MatchError(errors.New("Insufficient BOSH director version. The recreate-all errand requires a BOSH director version that satisfies one of the following: 266.15.0+, 267.9.0+, 268.2.2+ or 268.4.0+.")))
			}
