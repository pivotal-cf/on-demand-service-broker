...
package purge_instances_and_deregister_tests
...
var _ = Describe("purge instances and deregister broker", func() {
	...
	BeforeEach(func() {
		...
		Eventually(cf.Cf("create-service", serviceOffering, "dedicated-high-memory-vm", serviceInstance2), cf_helpers.CfTimeout).Should(gexec.Exit(0))
	...
	It("deletes and unbinds all service instances", func() {
		...
		Eventually(session, cf_helpers.CfTimeout).Should(gexec.Exit(1))
